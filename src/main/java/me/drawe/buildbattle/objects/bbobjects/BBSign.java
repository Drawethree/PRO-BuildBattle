...
public class BBSign {
    ...
    public BBSign(BBArena arena, Location loc) {
        ...
        try {
            ...
            BuildBattle.getInstance().warning("§cThere is no sign for arena §e" + arena.getName() + "§c in location §e" + LocationUtil.getStringFromLocationXYZ(loc) + " §c!");
            BuildBattle.getInstance().getFileManager().getConfig("signs.yml").get().set(arena.getName() + "." + LocationUtil.getStringFromLocationXYZ(loc), null);
            BuildBattle.getInstance().getFileManager().getConfig("signs.yml").save();
        }
    }
    ...
    public void removeSign(Player p) {
        BuildBattle.getInstance().getFileManager().getConfig("signs.yml").get().set(arena.getName() + "." + LocationUtil.getStringFromLocationXYZ(getLocation()), null);
        BuildBattle.getInstance().getFileManager().getConfig("signs.yml").save();
        arena.getArenaSigns().remove(this);
        blockBehind.setType(CompMaterial.AIR.getMaterial());
        if (p != null) {
            p.sendMessage(BuildBattle.getInstance().getSettings().getPrefix() + "§aSign for arena §e" + getArena().getName() + "§a successfully removed!");
        }
    }
