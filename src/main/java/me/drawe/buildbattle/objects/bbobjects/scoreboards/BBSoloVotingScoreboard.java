...
public class BBSoloVotingScoreboard extends BBScoreboard {
    ...
    public BBSoloVotingScoreboard() {
        super(BBGameMode.SOLO, BBArenaState.VOTING, BuildBattle.getFileManager().getConfig("translates.yml").get().getString("scoreboard.solo.voting.title"), BuildBattle.getFileManager().getConfig("translates.yml").get().getStringList("scoreboard.solo.voting.lines"));
    }
}
