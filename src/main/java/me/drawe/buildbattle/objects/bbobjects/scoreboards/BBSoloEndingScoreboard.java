...
public class BBSoloEndingScoreboard extends BBScoreboard {
    ...
    public BBSoloEndingScoreboard() {
        super(BBGameMode.SOLO, BBArenaState.ENDING, BuildBattle.getFileManager().getConfig("translates.yml").get().getString("scoreboard.solo.ending.title"), BuildBattle.getFileManager().getConfig("translates.yml").get().getStringList("scoreboard.solo.ending.lines"));
    }
}
