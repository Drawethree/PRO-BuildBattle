...
public class LeaderboardManager {
    ...
    public void selectLeaderboard(Player p) {
        ...
        if(selected != null) {
            p.sendMessage(GameManager.getPrefix() + " §aYou have selected Leaderboard with type §e" + selected.getType().name() + "§a at location §e" + LocationUtil.getStringFromLocation(selected.getLocation()));
    ...
    public void createLeaderboard(Player creator, Location loc, LeaderboardType type, int playerAmount, double refreshTime) {
        ...
        creator.sendMessage(GameManager.getPrefix() + " §aLeaderboard with type §e" + type.name() + " §acreated at location §e" + LocationUtil.getStringFromLocation(leaderboard.getLocation()) + "§a!");
        activeLeaderboards.add(leaderboard);
    ...
    private void saveLeaderboardIntoConfig(Leaderboard l) {
        BuildBattle.getFileManager().getConfig("leaderboards.yml").get().set("leaderboards." + LocationUtil.getStringFromLocation(l.getLocation()) + ".type", l.getType().name());
        BuildBattle.getFileManager().getConfig("leaderboards.yml").get().set("leaderboards." + LocationUtil.getStringFromLocation(l.getLocation()) + ".player-amount", l.getAmountToDisplay());
        BuildBattle.getFileManager().getConfig("leaderboards.yml").get().set("leaderboards." + LocationUtil.getStringFromLocation(l.getLocation()) + ".refresh-time", l.getRefreshTime());
