...
public enum CompMaterial {
        ...
        if (MinecraftVersion.atLeast(MinecraftVersion.V.v1_13)) {
            try {
                ...
                for (final CompMaterial xmat : CompMaterial.values()) {
                    if (xmat.legacyName.equals(item.getType().toString())) {
                        return xmat;
                    }
                }
    ...
    // Handles Minecraft below 1.12 and replaces non-existing Materials to AIR.
    private Material findName() {
        final String[] names = {name(), legacyName, alternativeName, "AIR"};
        ...
        throw new RuntimeException("[REPORT] CompMaterial could not parse " + this + ". Tried: " + String.join(", ", names));
    }
    ...
    public static CompMaterial fromStringStrict(String key) {
        ...
        Objects.requireNonNull(material, "Invalid material '" + key + "'! For valid names, see: https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/Material.html (Note that names change across MC versions!)");
        return material;
    }
...
class SoftMaterials {
    ...
    final static Set<String> MATERIALS = new HashSet<>(Sets.newHashSet(
            "SKULL",
            "ANVIL",
            "TRAPPED_CHEST",
            "GOLD_PLATE",
